...
while True:
    ...
    if comanda == 'EXIT':
        ...
        if not este_conectat:
            print("EROARE (local): Nu esti conectat. Foloseste CONNECT mai intai.")
            continue

        if len(parti) < 2 or not parti[1].strip():
            print("EROARE (local): Foloseste: PUBLISH <mesaj>")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
        ...
        if len(parti) < 2 or not parti[1].strip():
            print("EROARE (local): Foloseste: DELETE <id>")
            continue

        try:
            int(parti[1].strip())
        except ValueError:
            print("EROARE (local): ID-ul trebuie sa fie un numar intreg.")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
